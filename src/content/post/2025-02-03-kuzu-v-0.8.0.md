...
We’re kicking off 2025 with an exciting release - Kuzu 0.8.0!

This release brings a new feature that distinguishes Kùzu from any other graph database out there — Kùzu-WASM for in-browser graph analytics — run your graph database while keeping all data and compute within your browser session! WASM support enables client-side analytics and in-browser graph processing. This can be especially critical for handling sensitive data.

We’ve also added full-text-search (`fts`) to our extension ecosystem. With BM25-powered keyword search, you can now run text-based queries directly in Kùzu.

In addition to these new features, we’ve streamlined the developer workflow during relationship table creation by unifying `CREATE REL TABLE GROUP` into a single, flexible `CREATE REL TABLE` syntax. Finally, we’ve significantly improved our execution of aggregation queries via a new parallel hash aggregation mechanism. In the following sections, we’ll dive into more details.
...
we have developed a version of Kùzu Explorer, which is Kùzu's browser-based graph visualization and CLI. You can check out this
demo [here](https://demo-wasm.kuzudb.com/).
...
and allows Kùzu to answer queries very quickly (expect millisecond latencies).
