...
To use Kùzu-WASM, you have to install the `kuzu-wasm` package first `npm i kuzu-wasm` and then 
follow our [API documentation](https://docs.kuzudb.com/client-apis/wasm/) to integrate it into your projects. To see Kùzu-WASM in action,
we have developed a version of Kùzu Explorer, which is Kùzu's browser-based graph visualization and CLO. You can check out this
...
The index can be built on one or multiple columns in a Kùzu node tables. Similar to other
