...
To enable ultra-fast pattern matching in Kùzu, we leverage a graph-native approach: Words from source documents are stored as nodes, while their contextual occurrences are captured as relationships. The optimized graph structure, combined with our state-of-the-art query engine, allows Kùzu to retrieve patterns in milliseconds.
