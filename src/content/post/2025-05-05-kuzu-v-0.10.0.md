...
In v0.9, we introduced a parallel framework for recursive queries aiming to mitigate the long tail problem when recursive queries access a highly skewed node, i.e. node with high degree. This framework will blindly allocate a concurrent data structure whose size is linear to the number of nodes in relevant tables. This however, incurs an overhead when running a small recursive query on very large database in which case, the allocation becomes the bottleneck. 
...
```
MATCH (:Comment {id:<id>})-[:replyOf]->(:Comment) // 1ms
