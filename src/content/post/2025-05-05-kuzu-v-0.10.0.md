...
added a Neo4j migration extension, and as always, included performance optimizations to reduce our on-disk space usage 
for frequent data modifications, speed up recursive queries and JSON scans, and more.
...
Also we now support *(all) weighted shortest path* as built-in functionality.

More commonly-used algorithms are planned on the way in our future releases!

### Performance benchmarks

We'll demonstrate the performance and scalability of our graph algorithms in Kuzu on two
...
[^1]: Our Louvain implementation requires builiding in-memory graphs, which are memory-intensive, and requires more than the default buffer manager size on datagen-sf10k, so we omit Louvain in this dataset. This will be improved in our future releases.
...
Above benchmarks show our algorithms are able to perform well on billion scale graph and scales well as the number of threads increases.

### Using graph algorithms

All algorithms, except for that [(all) weighted shortest paths](https://docs.kuzudb.com/cypher/query-clauses/match#shortest-path) which is provided as built-in, are available via the [`algo` extension](https://docs.kuzudb.com/extensions/algos). 
...
Graph algorithms can also run on a subset of table records using a filtered graph. 
...
interested in using Kuzu's graph algorithms, give this a try, and check out the [docs](https://docs.kuzudb.com/extensions/algos) here.

## Neo4j migration extension 

A lot of Kuzu users have previously used or are using Neo4j. To make it easier to migrate graph data from Neo4j to Kuzu, we introduce a [`Neo4j`](https://docs.kuzudb.com/extensions/neo4j) extension that can automatically import nodes and relationships from Neo4j to Kuzu databases.
...
before running the migration into Kuzu. You can find more details on setting up the APOC extension on Neo4j desktop or server [here](https://docs.kuzudb.com/extensions/neo4j#set-up-neo4j-apoc-extension). Read more on Kuzu's Neo4j extension [here](https://docs.kuzudb.com/extensions/neo4j).
...
We're excited to now support Android devices with ARMv8-A architecture through our [Java API](https://docs.kuzudb.com/system-requirements/#java). 
For those who are interested in building mobile applications that leverage Kuzu's graph database capabilities directly on Android devices, please don't hesitate to give a try!
...
// Recursive
MATCH (:Comment {id:<id>})-[:replyOf*1..1]->(:Comment)
``` 
...
If you are inserting data from json sources or evaluating your json data on the fly, there should be a noticable 
performance improvement to your workload. 
