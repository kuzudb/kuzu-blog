...
- Strongly connected components (a parallel version & a kosaraju version)
- PageRank
...
| datagen-sf10k | 9.4B | 100M | |

We report the end-to-end runtime on a machine with <CONFIGURATION> usuing 4, 16 and 64 threads for execution.
...
Following the example above, assume we want to find influencers within a certain age group, e.g. 18-24. 
```cypher
...
CALL create_projected_graph(
    ...
    {
        'person': {
            'filter': 'n.birthday > date("2001-01-01") 
                AND n.birthday =  date("2007-12-31")'
        }
    },
    ['knows']
);
...
Reusing disk space has been a nice but long missed feature in Kuzu.
It can prevent database size from bloating after frequent data modifications.
...
- We never shrink our `data.kz` file, but only keeps track of free space within the file and try to reuse in the future.
...
1. **Multi COPY**: We partition the `person` csv file into `x` partitions, each of which is saved as a csv file. We copy each partitioned csv file one at a time, and report the final size of `data.kz` file.
...
In v0.8 we introduced a parallel framework for recursive queries aiming to mitigate the long tail problem when recursive queries access a highly skewed node, i.e. node with high degree. This framework will blindly allocate a concurrent data structure whose size is linear to the number of nodes in relevant tables. This however, incurs an overhead when running a small recursive query on very large database. In such case, the allocation becomes the bottleneck. We mitigate this problem by starting with a small allocation and dynamically grow the data structure.
Below shows a micro benchmark on recursive query performance on small and large queries on a large database. 

TODO: Xiyang add benchmark numbers
