...
- **Free-space management:** We have introduced a new free space management mechanism that reclaims disk space usage. This will
make some workloads get much faster!
...
using the `WSHORTEST` keyword, e.g., `()-[:* WSHORTEST ... ]->()`
See the documentation for weighted shortest paths [here](XXX).  (TODO: Fix the Cypher WSHORTEST pattern
and the XXX in the reference).
