...
queries that are aligned with both the user's question and the graph schema, and once the relevant data is
retrieved from the graph, this can provide useful context to an agent for downstream tasks.

Real-world graphs at enterprises can be quite complex: many node types, many-to-many relationships, and lots of properties.
...
- **Maximize** the mutual information[^2] between $Y$ and $C$ -- i.e., the given $C$ has a high probability of producing $Y$
- **Maximize** the amount of _relevant_ information in $R$
- **Minimize** the amount of information in $N$ and $S$ that the model must process to arrive at $Y$

By understanding the context in terms of these components, we can think about how to reduce the chances of
the model hallucinating or getting misled by the context to produce the wrong answer.
...
the user's question, and importantly, the graph schema. Unfortunately, real-world graph schemas are often
...
quality of the Cypher query generated, and discuss some of the trade-offs involved.

## Experiments

For the experiments below, we'll use the [LDBC Social Network Benchmark](https://ldbcouncil.org/ldbc_snb_docs/) (LDBC-1) dataset, which is a synthetic
graph dataset that's commonly used to measure the performance of graph databases. The schema of the LDBC social network graph is reasonably
...
are 23 relationship labels and 7 node labels, each with numerous properties. Pictorially, the schema looks like this:
...
to help you get a sense of how the schema is represented in the LLM prompt. The schema represents the structure of the graph
(what directions the relationships can go in), as well as the properties are available on each node/relationship label.
...
the LLM is able to appropriately join on the right nodes and relationships to answer the question.
...
BAML makes it simple to get high-quality structured outputs due to its emphasis on "schema engineering" via
an expressive type system.

The Text2Cypher prompt in BAML (after a few iterations) looks like this:

```rs
// Output type: Cypher query with no newlines
class Query {
...
Kuzu-specific instructions baked in.
...
Text2Cypher on each of the 30 questions in the test suite. All three formats of the graph schema are tested:
JSON, XML and YAML. The passing/failing tests are colour-coded and plotted as a heatmap to help visualize
...
When we pass in the full graph schema as part of the prompts to recent models, we can see that the large, more capable
models like `openai/gpt-4.1` and `google/gemini-2.0-flash` are able to pass
many more tests than smaller models like `openai/gpt-4.1-nano`. Very new (at the time of writing) models like
...
we saw that the most common errors were related to the LLMs inverting the direction
of the relationships, despite the system instructions explicitly stating that the direction is important
and that the LLM should focus on the `from` and `to` labels of the relationships.

Below is a query that `google/gemini-2.0-flash` generated, and it _almost_ got it right -- the only mistake is that
...
This is an indication that the full graph schema's verbosity leads to "context confusion", where the LLM
is unable to focus sufficiently on the most relevant parts of the context to achieve its goal.

### Pruned graph schema results

As a next step, we pass the original (full) schema to another BAML prompt whose **only** task
is to prune the schema to retain the most relevant parts based on the user's question. For example,
if the question is asking about the number of comments created by a person in a particular place,
...
Because BAML provides an expressive type system,
we can define custom types to represent the schema exactly as we want. In this case, the `GraphSchema`
type is defined as a collection of `Node` and `Relationship` types that we want our pruning prompt to
output. This way, the output of the pruning prompt from BAML has the _same_ shape as the JSON
representation we obtain from Kuzu via Cypher, allowing us to reuse code very effectively. Very useful!
...
Despite the schema being very small compared to the full schema, the LLM still fails to get the direction
...
All in all, it's very clear that schema pruning is a very effective way to improve the quality of the Cypher queries
generated by Text2Cypher pipelines.

## Recommendations for better Text2Cypher

In this section, we summarize some of the lessons learned from the experiments we ran and provide
some recommendations to get the best of your LLMs when doing Text2Cypher.

### Spend time on schema design

Because LLMs are trained on a lot of natural language data, they tend to capture semantics _far better_
when the paths read more like a sentence in natural language. Spend time on designing the schema to
help both humans and LLMs. Chances are, the LLM will correctly interpret the `from`/`to` directions
...
The following table shows some examples of how the schema can be designed to help the LLM.

| Example | How it looks to the LLM | Recommendation |
|------------|---------|:-------:|
...
Somewhat contrary to popular wisdom, it's better to start with a large, capable model and understand the capabilities
of the current state-of-the-art, before embarking on a premature prompt engineering effort to get the most out of smaller models.
...
into a test suite _early on_ in the development process. Even a small test suite of 30-50 questions, as we showed
...
(similar-sounding property names, similar labels, etc.), allowing the model to focus more on the task
at hand. Adding an additional pruning step upstream of Text2Cypher does increase latency by ~2x, but
for the massive improvement in query quality (at least for the larger models), it's well worth the trade-off.

### Iterate on the full prompt

Carefully inspect and iterate on the entire prompt (including system instructions) to ensure there
...
theory mindset outlined above, you'll find the context engineering process a lot more intuitive and
...
as a prompt format instead of JSON/YAML. Initial experiments (see [here](https://github.com/kuzudb/text2cypher/blob/main/query_graph_ddl_schema.py)
for the code) with pruning the DDL schema are promising, but more prompt engineering is needed to get
this working on a wider range of questions.
...
to automatically optimize the prompts, based on training data of good and bad examples. From the experiments
...
or `openai/gpt-4.1-nano` can get better at Text2Cypher. These optimized prompts are typically
far superior to prompts written by a human, so this is a very promising direction to explore in the future[^4].

## Conclusions

As the AI landscape continues to evolve, it's very obvious that models' capabilities will continue improving
at a very rapid pace, and the cost of running them (as well as latency) will continue to decrease. The trade-offs
among these different approaches will be a moving target, but with exciting new capabilities
such as prompt optimization and fine-tuning becoming more and more accessible, it's a great time to be
exploring ways to get models to write better Cypher, too!
