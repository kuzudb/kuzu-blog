...
These changes have led to **significant** performance improvements in data insertion. Below, we show the performance difference from the previous version (0.4.2) when doing 100,000 insertions into a node table.
...
We observed a **15x** speedup in this case, though the exact speedup you observe may vary depending on your write workload.
...
In this section, we describe some of the new features and data types that we've added to Kùzu in this release.
...
cases where the equivalent functionality doesn't exist in Cypher. It's still recommended to first look for an
equivalent Cypher function in Kùzu before writing your own UDFs in Python, as there are performance implications
to running pure-Python functions in your application. If you find that your workflow isn't meeting
performance requirements and you have a commonly used query pattern for which an internal Kùzu function
doesn't exist, reach out to us on [Discord](https://kuzudb.com/chat) to discuss this further.
...
We think that this feature will be very useful to users who are exploring and transforming their data in
Python via DataFrames, and we hope that it will make your workflows more efficient and flexible.
...
which works the same way as it does in other systems. Decimal data type is defined as
...
Some additional functions and features have been added to the DDL in this release. Below, we list some of the
important ones.
...
exists in the database. Now, you can use `CREATE ... TABLE IF NOT EXISTS` to avoid a RunTime error being
raised.
...
Just like the previous case, you can now drop a table without raising an exception if it doesn't exist.
...
In this release, we provide lambda functions that can be used like you may be used to from other languages.
...
In certain cases, Kùzu might generate a sub-optimal join order when the obtained statistics are inaccurate at the optimization stage of a large graph query. We provide join order hints to allow users to enforce a specific join order that bypasses Kùzu’s optimizer.
...
Our many thanks go out to the entire team at Kùzu Inc. for their hard work in making this release possible. Hopefully,
...
In the meantime, we recommend that you give fire up a database in the newly released version and try these
features out for yourself. Do let us know if you have any feedback on [Discord](https://discord.gg/VtX2gw9Rug),
