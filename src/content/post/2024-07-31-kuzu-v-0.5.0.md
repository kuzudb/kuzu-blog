...
where `x` is the threshold value in MB.
...
It also allows attaching to remote Kùzu databases, which is a new feature that is part of this release (see [below](#remote-kùzu-databases) for more on this).
...
To illustrate the performance gains after enabling local file cache, here is a benchmark which scans an LDBC-10 `Comment` table
which is of size 2.1GB and is stored in a remote S3 bucket.
The benchmark is run on a machine with 2 x AMD EPYC 7551 servers.
...
the whole file remotely. This takes 1402s. Below are the performance numbers when turning the `HTTP_CACHE_FILE` option on.
...
The first query after we start the transaction takes 32s, already a major improvement (**43x**). The compilation time takes
27.5s, which is primarily the time to download a copy of the file with one request to S3. Then the further execution
time takes 5s, which is the time to scan from the downloaded local disk. The second `LOAD FROM` from takes
only 5.2s (an additional 6.3x gain) since it's completely executed locally, i.e., no downloading takes place.

## Features

We have also added many new features in this release. Below are the main ones.
...
See this [documentation page](https://docs.kuzudb.com/extensions/httpfs#attach-remote-kuzu-database)
...
`COPY FROM` copies external data or results of sub-queries into a Kùzu node or relationship table. 
In prior releases of Kùzu, if you wanted to copy the data in Pandas DataFrame to a Kùzu table using `COPY FROM`, you
...
Checkout more details in this [documentation page](https://docs.kuzudb.com/extensions/json). Soon, we also plan to release a feature
that provides a JSON data type in the system, that can be used to store JSON documents as node or relationship properties.
...
We added a [`SEQUENCE` generator](https://docs.kuzudb.com/cypher/data-definition/create-sequence) to the DDL to auto-generate sequences of numbers. The range and
...
Whew, that was a lot of new features and improvements! As always, we've only scratched the surface of listing them in
