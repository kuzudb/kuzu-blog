...
new features, such as attaching to remote Kùzu and SQLite databases, Python UDFs, JSON support, scanning from
Polars DataFrames and PyArrow tables, user-defined data types, and for a bonus usability feature: a progress bar
in the CLI and Kùzu Explorer! Without any further ado, let's dive in!
...
disk I/O and could trigger re-compressing column chunks, leading to slowdowns.
...
Kùzu is designed to be very performant on read-heavy query workloads, moving to MVCC is also making Kùzu performant on
...
You can use the [remote file system cache feature](https://docs.kuzudb.com/extensions/httpfs#attach-remote-kuzu-database) (i.e., run `HTTP_CACHE_FILE=true`) to cache these remote Kùzu databases to improve 
your query performance, similar to how you can cache other remote databases and files.

See this [documentation page](https://docs.kuzudb.com/extensions/httpfs#attach-remote-kuzu-database) for more information on connecting to remote Kùzu databases.
...
An example of the new feature allowing you to scan from a Polars DataFrame is shown below:
...
You can also scan from in-memory PyArrow tables [the same way](https://docs.kuzudb.com/import/copy-from-dataframe#arrow-tables).
...
You can now define [user-defined data types](https://docs.kuzudb.com/cypher/data-types#user-defined-types) by leveraging the built-in primitive types. As a simple toy example, you can define a
type `AGE_T` that is based on `INT8` as follows:
...
complex data types, e.g., `STRUCT`s, that you define. For example, the below `CREATE NODE TABLE` statement
uses the `AGE_T` type in a `personInfo` struct:

```sql
CREATE NODE TABLE Person (id INT64, personInfo STRUCT(name STRING, age AGE_T), PRIMARY KEY(id));
```
