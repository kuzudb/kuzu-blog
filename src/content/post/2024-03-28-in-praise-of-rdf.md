...
- **RDF overview**: RDF is a simple and flexible data model, where
  ...
  that give clear meanings to certain vocabulary in triples (especially the verbs).
...
data model. For reasons I will explain, it is considered a graph-based model.

[^2]: Here is a fun quote on the distinction between "data" and "knowledge" from
[Jeff Ullman's](https://en.wikipedia.org/wiki/Jeffrey_Ullman)
...
modeling will just not work. Let's take the *extremely ambitious goal* of digitizing the knowledge
on Wikipedia so people can use it in software applications. This is the goal of projects/datasets like 
...
under hierarchies of classes and categories with irregular sets of properties 
...
in both under 'Wearables' and 'Electronics' through a chain of `rdf:type` and `rdfs:subClassOf` relationships.
- Entities can have irregular sets of properties. 'Levi's 511' has 2 sizes while
  'Levi's 512' has only one. 'Levi's 511' has four different properties, `gc:desc`, `gc:size`, and `gc:price`, `gc:contains`
...
- Flexibility: The relational model is located as the left most point because it is the most "rigid", the most
structured data model. RDF is the right-most point because it is the most flexible model. 
Original property graph (PG) model of Neo4j falls 
...
homogenous, e.g., if every "Jeans" has exactly one "size" property and "size" properties are all of the same type, 
...
As a side note: Kùzu implements a variant of the property graph data model we call *structured PG* model. This is 
equivalent to the relational model, except we require users to identify their tables as "node" or
"relationship" tables. That is why this model appears as the same point as the relational model in the figure.
...
tables, users tell us something important about their workloads. Specifically, they inform Kùzu
that the relationship records will be used to join node records with their "neighbor" node records.
This is true in original PG DBMSs as well. They also have the same information.
...
that committees in [W3C standards](https://www.w3.org/standards/) community set down and gave specific 
...
As every other high-level query language, it is inspired by SQL but tailored to query triples. 

I will give one example here from an RDFS term `rdfs:subClassOf` to clarify what I mean by RDFS and OWL
...
[^4]: For learning more about KRR, I highly recommend this well-written and accessible book 
...
to do this in SQL. Forgetting about the fact there is no automatic inference capability in RDBMSs,
...
with the tuple, so something like ("Levi's 511", 20, "Jeans") but you're now going towards an RDF-like modeling.
...
There are standardized vocabulary other than `owl:sameAs` to integrate data across databases.
...
The second vice is that recall RDF is particularly flexible to model complex domains of information.
That means it is inherent that if you are really going to be that ambitious and construct large
...
However, it is gaining more prominence lately 
...
another direction to improve LLMs is to combine them with systems that do reasoning,
...
Cyc is a symbolic AI system that has common sense knowledge
about the world based on millions of rules, such as "no person can be at more than
one different place at the same time", or that "people believe that cats can breathe".
...
Lenat describes 5 different opportunities LLMs can benefit from 
a symbolic AI that reasons using the knowledge in a knowledge graph/base.
For example, it can double check if the LLM's arguments are logically consistent 
(his 1st opportunity). Or when an LLM generates a conclusion (without argument),
...
But given the increase in symbolic AI, this has become a relevant topic again.
...
of 80s and 90s, was to develop systems that had such reasoning capabilities (though
not at the level in this example).[^9]
Given the dominance of AI these years and the need to
...
called Description Logics, so cannot express a lot of constraints and rules.
Cyc, as Lenat describes in his article, is based on a language called [CycL](https://en.wikipedia.org/wiki/CycL),
that covers full first-order logic.
...
but still do not have means to express the constraints of RDFS or OWL, so
