...
## Vector Index
With the growing demand for vector similarity searches in databases, we are excited to introduce the new [`VECTOR`](https://docs.kuzudb.com/extensions/vector/) extension, which enables efficient similarity searches over vector data in Kuzu.

We employ a [hierachical navigable small world (HNSW)](https://dl.acm.org/doi/10.1109/tpami.2018.2889473) vector index design with two layers:
- The lower layer consists of all vectors (represented as nodes) connected by edges to their nearest neighbours.
- The upper layer is same as the lower layer in structure, except for that it only consists of a sampled subset of the vectors.

Since each layer is inherently a graph, we store them as relationship tables in Kuzu.
In this way, we are able to leverage Kuzu's native storage and query capabilities for efficient indexing and search operations.
...
The extension provides three core functions:
```cypher
...
Using our Python API, we demonstrate how to index and search book titles based on their semantic similarity.
...
To perform a filtered search, define a filter using a projected graph:
```cypher
CALL CREATE_PROJECTED_GRAPH(
    'projected_graph_name', // Name of the projected graph
    { 'table_name': {'filter': 'predicate' } }, // Node filters
    [ 'table_name' ] // Relationship tables
);
``` 

A projected graph is a subgraph containing only nodes and relationships that match the given table names and filters. 
Currently, projected graphs support single node tables with simple property-based filters. 
More flexible filtering using Cypher variables will be supported in future updates.

For example, to find publishers of books similar to "The Quantum World" published after 2010:
```python
...
### Performance
We evaluate the index construction and query performance on [ann-benchmark](https://github.com/erikbern/ann-benchmarks).
Benchmarks were run on a Mac Mini with an M4 Pro chip, 64GB RAM, and a 1TB SSD.
Kuzu is set to on-disk mode and we set its max num threads to **8**,
...
It is shown from the above table that under various datasets, the average query latency remains under 8ms.

To evaluate filtered search performance, we apply a filter on the base table, adjusting its selectivity to 1%, 2%, 3%, 5%, 10%, 20%, 30%, 50%, 75%, and 90%.
We randomly select 50 queries from the GIST query set. For each selectivity level, we apply the filter using a projected graph and perform the search.
...
In v0.8.0, we introduced a WebAssembly (WASM) version of Kuzu. One limitation of the initial WASM release was the lack of support for extensions. This release addresses that by bundling several extensions through static linking. The bundled extensions are
- Full-text search (FTS)
- Json (JSON)
- Vector Index (VECTOR)

After the release, you can directly use any functionality of `FTS`, `JSON` and `VECTOR` extensions in WASM without explicitly `LOAD` them.
...
The performance of index creationg is also improved in this release. The following benchmark is conducted using `ms-passage` dataset with 8.8M documents that take 2.9GB in raw size and on a machine with 2 AMD EPYC 7551 CPUs with 64 cores and 409GB of memory. The result is shown below that the index creation time has been improved by **4.3x**!
