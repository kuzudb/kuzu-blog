...
This data type is not natively supported by Kùzu, but is available if you install the [JSON extension](https://docs.kuzudb.com/extensions/json/).
...
Kùzu can now automatically detect the CSV configurations, such as delimiter, quote, and escape character,
...
## Removal of RDFGraphs

After much deliberation, we have removed our RDFGraphs feature from Kùzu beginning from this release.
We had originally added this feature to enable users to import and query their RDF data in Kùzu,
but for reasons of maintainability (and to keep our database binary small in size), we decided to remove it
from the core system and instead repackage it as an extension. The
technical reasons for this decision are [here](https://github.com/kuzudb/kuzu/issues/4289), but users
