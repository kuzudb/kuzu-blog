...
title: "Kùzu and the Connected Data London Knowledge Graph challenge"
description: "How we used Kùzu to build a Graph RAG pipeline for the Connected Data London KG challenge"
pubDate: "Dec 18 2024"
...
It's been an eventful end to the year for Kùzu Inc.! Some members of our team were in London 🇬🇧 in early December to participate in [Connected Data London 2024](https://www.connected-data.london/),
...
From a Kùzu perspective, one of the highlights of the event was the
...
using Kùzu as the underlying graph store, and how we built a Graph RAG pipeline on top of it. Along the way,
I hope it'll become clear how useful Kùzu can be for facilitating the rapid iteration and experimentation
...
- Graph construction: Construct a Kùzu graph using a combination of hand-curated domain knowledge,
...
In our case, we choose to use Kùzu as the graph database downstream, but the metamodel's
...
Kùzu is an embeddable property graph database that supports Cypher. Once the metamodel is defined as shown
above, it's relatively straightforward to transform the metamodel into a property graph schema in Kùzu
that we can then use to construct the graph. Visually, the property graph schema in Kùzu looks like the following:
...
The domain graph is constructed and stored in Kùzu and can be visualized in [Kùzu explorer](https://docs.kuzudb.com/visualization/).
...
Let's now use the Kùzu graph to answer some questions about our data! The Graph RAG pipeline consists of a two-stage process:
...
chances of a match in the graph. Q3 asks about the a specific event, Connected Data World 2021, which
is a node in the graph. As can be seen, the LLM correctly writes a Cypher query that matches on the
node and returns the event's description. Our prompt engineering worked reasonably well for these
questions!

That being said, the above Graph RAG pipeline has its limitations. If the user question is phrased
using terminology that's not present in the graph, the LLM may not be able to resolve the question
correctly and it may write an incorrect Cyphe query, resulting an empty response from the retrieval.
In such cases, some fallback mechanism is needed to either rewrite the query in a different form,
or to use an additional retrieval (for e.g., vector or keyword-based search) to answer the question.
...
There are numerous other ways to further improve on the variety of questions that can be answered
using the Graph RAG methodology shown. Some examples include:
...
All in all, the CDKG challenge was a fun and interesting project to cap off the year, and I'm really glad to have
...
of the CDKG challenge) and does not refer to a specific data model. Any reference to "knowledge graph" in relation to Kùzu refers to the
underlying property graph in Kùzu.
...
in [this file](https://github.com/Connected-Data/cdkg-challenge/blob/main/src/kuzu/rag.py). The temperature is once again set to 0.0
...
and functions are specified in the user prompt to help the LLM generate syntactically correct Cypher that runs in Kùzu -- for example,
...
[^4]: The system and user prompts used for the `gpt-4o-mini` LLM that generates the response to the question can be found in the
same [file](https://github.com/Connected-Data/cdkg-challenge/blob/main/src/kuzu/rag.py). For text generation, we set the temperature to
...
catalog the various Graph RAG patterns and methodologies. The vocabulary used in this post is inspired by the terminology
used in the larger Graph RAG community, so we encourage you to read more about the different patterns and techniques that
are defined in the site.
