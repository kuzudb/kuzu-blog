...
It's been a flurry of internal activity since our last release, and we're delighted to
...
Kuzu's [`vector`](https://docs.kuzudb.com/extensions/vector/) index is offered as an extension,
and implements the well-known [hierachical navigable small world (HNSW)](https://dl.acm.org/doi/10.1109/tpami.2018.2889473)
vector index with two layers. Notably (and unlike many other systems), Kuzu's implementation of HNSW is **disk-based**,
...
so we're able to store them using relationship tables in Kuzu, allowing us to make use of Kuzu's fast query capabilities.

We implemented a novel adaptive search algorithm that is predicate-agnostic and offers robust search performance.
The algorithm is based on recent research done by [Gaurav Sehgal](https://www.linkedin.com/in/gaurav-sehgal-79abb9112/)
and [Semih Salihoglu](https://cs.uwaterloo.ca/~ssalihog/), and is currently under review for publication.
We'll publish another blog post to explain many more technical details about the implementation details of our vector index
in the future, so stay tuned!

For this release post, let's illustrate how the vector index works using the example below in Python.
...
It's simple to create avector index on the `title_embedding` column of the `Book` table:
```python
# Create vector index
conn.execute(
    """
...
result = conn.execute("""
    ...
    RETURN nn.title ORDER BY distance;
    """,
...
subset of records (filtered search). To perform a filtered search, we use a [projected graph](https://dev-docs.kuzudb.com/extensions/vector/#what-is-a-projected-graph)
...
In this example, we'll use a projected graph to find publishers of books similar to "The Quantum World"
...
- We currently only support index over `FLOAT` (32-bit float) array columns in Kuzu. Support for 64-bit floats (`DOUBLE`) will be added in the next release.
...
With the frenzy of activity around the Model Context Protocol (MCP), an open protocol released by Anthropic
to standardize how LLMs interact with external tools, Kuzu is happy to now provide an
...
significantly improved in this release. The following benchmark is conducted using `ms-passage` dataset
with 8.8M documents that take 2.9GB in raw size and on a machine with 2 AMD EPYC 7551 CPUs with 64
